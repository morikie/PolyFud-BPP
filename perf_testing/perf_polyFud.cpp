...
int main (int argc, char * argv[]) {
	...
	std::unordered_map<std::string, double> thresholdMap = {
		{std::string("aataaa"), 0.00 },
		{std::string("attaaa"), 0.00 },
		{std::string("tataaa"), 0.00 },
		{std::string("agtaaa"), 0.00 },
		{std::string("aagaaa"), 0.00 },
		{std::string("aatata"), 0.00 },
		{std::string("aataca"), 0.00 },
		{std::string("cataaa"), 0.00 },
		{std::string("gataaa"), 0.00 },
		{std::string("aatgaa"), 0.00 },
		{std::string("actaaa"), 0.00 },
		{std::string("aataga"), 0.00 }
	};
	...
	for (size_t i = 0; i < 42; i++) {
		...
		for (auto vecIt = pasPosAndSeqTP.begin(); vecIt != pasPosAndSeqTP.end(); vecIt++) { 
			PolyFud u3Fuzzy(vecIt->second, searchBackward);	
